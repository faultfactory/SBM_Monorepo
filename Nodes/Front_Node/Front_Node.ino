...
void loop()
{
  ...
  if(CAN_Send_Delay.justFinished()) // Checking the delay on each loops
  {
    ...
    steerVal = map(steerRaw,2,1023,0,270);
  ...
  /**********************************************************************************************/
    ...
    for (int i = 0; i < numTrans; i++) {
     ...
     display.setCursor(40,20); display.println(steerVal);
     display.setCursor(40,30); display.println(analogRead(Voutput));
     ...
     pack_message(&can_obj,0x103,&msg_103.a);
