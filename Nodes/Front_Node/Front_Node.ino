...
void loop()
{
  ...
  /**********************************************************************************************/
    ...
    for (int i = 0; i < numTrans; i++) {
      ...
      Pfilter[i] = aLPF*Pfilter[i] + (1 - aLPF)*Papplied[i]; // Low pass filter (Pressure), NOT SURE IF I SHOULD DO THIS AGAIN!!! [REMOVE? CHECK RESPONSE TIME]
    }

     // Print information to serial monitor & OLED
     Serial.print(Pfilter[0]); Serial.print(" "); Serial.println(Pfilter[1]);
     display.clearDisplay();
     ...
     display.setCursor(40,0); display.println(Pfilter[0]);
     display.setCursor(40,10); display.println(Pfilter[1]);
     display.setCursor(40,20); display.println((Pfilter[0]/(Pfilter[0] + Pfilter[1]))*100);
     display.display();  
     /**********************************************************************************************/
